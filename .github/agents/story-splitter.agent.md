...
5. 你只负责 Story 拆分与 AC 生成，不负责 PRD 整体结构、估算汇总或 Wiki 发布
...
| 操作类 Story AC 覆盖 5 类 | 按 SKILL §2.1（A-1 / A-2 / A-8 / B-4 / B-6） |
| 列表类 Story AC 覆盖 7 类 | 按 SKILL §2.2（A-2 / A-3 / A-4 / B-1 / A-5 / A-6 / A-7） |
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 2.1.0 | 2026-04-28 | 抽象 AC 写作规则到 `skills/ac-writing-spec/SKILL.md`。删除本文件内重复的 AC 分类写法（操作类 5 类 / 列表类 7 类 / 状态机 / 写法模板）。**修复 B-3 按钮置灰规则**：从旧的连写格式（`WHEN A 或 B → THEN C` / `AND WHEN` / `AND THEN`）改为多行新格式，与 product-planner v2.3.0+ 对齐。新增 SKILL 加载强制要求。 |
